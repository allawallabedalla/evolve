...
export class World {
  ...
  migration: number[][] = [];
  private popConfig: Partial<PopulationConfig>;
  private rng: () => number;
  ...
  addPlace(name: string, env: Environment, start = 0.5): Place {
    ...
    this.migration = Array.from({ length: n }, (_, i) =>
      Array.from({ length: n }, (_, j) => this.migration[i]?.[j] ?? 0),
    );
    return place;
  }

  /** Symmetrische Verbindung (Migrations-Rate in beide Richtungen). */
  connect(i: number, j: number, rate: number): void {
    this.migration[i][j] = rate;
    this.migration[j][i] = rate;
  }

  /** Verbindung kappen (Isolation). */
  isolate(i: number, j: number): void {
  ...
  step(): void {
    for (const p of this.places) p.pop.step(p.env, this.phys);
    this.migrate();
  }
   ...
   */
  colonize(from: number, to: number, founders = 5): void {
    const src = this.places[from].pop.genomes;
    const N = this.places[to].pop.size;
    const seeds: number[][] = [];
    for (let k = 0; k < founders; k++) seeds.push(src[(this.rng() * src.length) | 0].slice());
    const next: number[][] = new Array(N);
    for (let k = 0; k < N; k++) next[k] = seeds[(this.rng() * founders) | 0].slice();
   ...
   */
  catastrophe(i: number, survivorFraction = 0.05): void {
    const pop = this.places[i].pop;
    const N = pop.size;
    const s = Math.max(1, Math.round(survivorFraction * N));
    const survivors: number[][] = [];
    for (let k = 0; k < s; k++) survivors.push(pop.genomes[(this.rng() * N) | 0].slice());
    const next: number[][] = new Array(N);
    for (let k = 0; k < N; k++) next[k] = survivors[(this.rng() * s) | 0].slice();
    pop.genomes = next;
  }

  /** Mittlere paarweise Genom-Distanz einer Population (Diversitäts-Mass). */
  diversity(i: number): number {
   ...
   */
  diversityNN(i: number, sample = 80): number {
    const g = this.places[i].pop.genomes;
    const N = g.length;
    const G = g[0].length;
    const step = Math.max(1, Math.floor(N / sample));
    const idx: number[] = [];
    for (let a = 0; a < N; a += step) idx.push(a);
    let sum = 0;
    for (const a of idx) {
      let best = Infinity;
      for (const b of idx) {
...
export function meanDistance(a: number[], b: number[]): number {
  let s = 0;
