...
export function modes1D(values, opts = {}) {
    ...
    for (const m of maxima) {
        ...
        let valley = Infinity;
        for (let b = lastKept; b <= m; b++)
            valley = Math.min(valley, dens[b]);
 ...
 */
export function selectionWeights(mean, env, phys) {
    const eps = 0.01;
    const G = mean.length;
    const w = new Array(G);
    for (let g = 0; g < G; g++) {
 ...
 */
export function clusters(genomes, opts = {}) {
    ...
    for (const seed of order) {
        ...
        const centroid = new Array(G).fill(0);
        for (const m of members)
            for (let g = 0; g < G; g++)
                centroid[g] += genomes[m][g] / members.length;
